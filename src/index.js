...
export class EveryCredVerifier {
    ...
    credentialIssuerValidation = false;
    ...
    async validateCredentials() {
        this.credentialValidation = new CredentialValidator().validate(this.certificate);
        if (this.credentialValidation) {
            let data = await new CredentialIssuerValidator().validate(this.certificate);
     ...
     */
    async revocationStatusCheck() {
        this.credentialIssuerValidation = await new RevocationStatusCheck().validate(this.revocationListData, this.certificate, this.issuerProfileData);
    ...
    async validateChecksum() {
        this.checksumValidation = await new MerkleProofValidator2019().validate(this.certificate);
