...
export class MerkleProofValidator2019 {
    credential;
    decodedData;
    normalizedDecodedData;
    blockchainApiResponse;
    isMerkleProofVerified = false;
    constructor() { }
    /**
    ...
    async validate(credentialData) {
        ...
        this.decodedData = getDataFromKey(this.normalizedDecodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.decoded_proof_value);
        const isSignatureValid = (this.checkDecodedAnchors() &&
        ...
        if (isSignatureValid &&
            ...
            const normalizedData = getDataFromKey(this.normalizedDecodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.get_byte_array_to_issue);
     ...
     */
    validateNormalizedDecodedData(response) {
        logger(Messages.FETCHING_NORMALIZED_DECODED_DATA);
        if (!isKeyPresent(response, CHECKSUM_MERKLEPROOF_CHECK_KEYS.decoded_proof_value) &&
     ...
     */
    async getNormalizedDecodedData() {
     ...
     */
    async verifyMerkleProof(decodedData) {
    ...
    checkDecodedAnchors() {
        ...
        if (isKeyPresent(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.anchors)) {
            const anchorsData = getDataFromKey(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.anchors);
    ...
    checkDecodedPath() {
        ...
        if (isKeyPresent(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.path)) {
     ...
     */
    checkDecodedMerkleRoot() {
        logger(Messages.MERKLEROOT_DECODED_DATA_KEY_VALIDATE);
        if (isKeyPresent(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.merkleRoot)) {
            const merkleRootData = getDataFromKey(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.merkleRoot);
     ...
     */
    checkDecodedTargetHash() {
        logger(Messages.TARGETHASH_DECODED_DATA_KEY_VALIDATE);
        if (isKeyPresent(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.targetHash)) {
            const targetHashData = getDataFromKey(this.decodedData, CHECKSUM_MERKLEPROOF_CHECK_KEYS.targetHash);
